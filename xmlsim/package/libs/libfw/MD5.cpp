...
void MD5::update(FILE *file)
{
    ...
    unsigned char l_buffer[1024];
    int len;

    while (len = fread(l_buffer, 1, 1024, file))
        update(l_buffer, len);
...
void MD5::update(istream& stream)
{
    ...
    while (stream.good())
    {
        stream.read(l_buffer, 1024); // note that return value of read is unusable.
        len = stream.gcount();
        update((unsigned char*)l_buffer, len);
    }
...
char* MD5::hex_digest()
{
    ...
    if (!finalized)
    {
        ...
        "finalized the digest!" << endl;
        return const_cast<char*>(szBlank);
    }
